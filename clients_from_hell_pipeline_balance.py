...
temp = df.query('deadbeats == 0').sample(871)
temp_d = df.query('deadbeats  == 1')
df_down_sample = pd.concat([temp, temp_d])

st.title("Balance of the target")
plt.hist(df_down_sample['deadbeats'], bins = 2, rwidth = 0.5)
st.pyplot()
...
k_fold(preprocess(df_down_sample), target = 'deadbeats')
